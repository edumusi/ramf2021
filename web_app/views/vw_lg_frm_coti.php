...
<script>
  ...
  { 
	...
	{
		var index;
		var servicio 	= ["64","65","66","88","89","90"];
		...
		{	
                    ...
                    $(".ventas"+servicio[index]).change(function(){ totalVentaServicio("ventas"+servicio[index]); });
		}
			
		$('.boton_confirm').addClass('pointer');	
		$('.boton_confirm').click(function() {
			var r = confirm("¿Borrar Cotización en PDF?");
			if (r == true) { borraCotizacionAX(href,$(this).attr('id')); }
			});		
	}
  ...
  $tc    = NULL;
   ...
   if($accion == "N")
	...
	if($cotizacion['servicios'][0] == NULL)
        {
            $celda = '  <div class="row" id="servicio64"></div>
	...
	{       $x 	   = -1;
		...
		for ($s = 0; $s <= 5; $s++) // los tres tipos de servicios 64,65,66,88,89,90
		{  
                    $x              = $x + 1;
		 ...
		 {	
			...
			$servicios = $servicios.$saltoLinea.
				  ...
				  "<tr><td width='100%' align='center' colspan='4' style='font-weight:bold; background-color:#DC2428; color:#FFFFFF'> ".$ts."</td></tr>".
				  "<tr><td width='10%'></td><td width='80%' colspan='2' align='center' style='font-weight:bold; background-color:#DC2428; color:#FFFFFF'>TERMINOS</td>".
      ...
      $myRow = "";
			...
			$servicios = $servicios.$myRow."</table>".$saltoLinea.
				  ...
				  "<tr ><td width='10%'></td><td width='80%' colspan='2' align='center' style='font-weight:bold; background-color:#DC2428; color:#FFFFFF'>DETALLE DEL EMBARQUE</td>".
				  "<td width='10%' align='center'> ".
			...
			$servicios = $servicios.$myRow."</table>".$saltoLinea.
				  ...
				  " <tr>".
				  " <td width='45%' align='center' style='font-weight:bold; background-color:#DC2428; color:#FFFFFF'>CONCEPTO</td>".
				  " <td width='45%' align='center' style='font-weight:bold; background-color:#DC2428; color:#FFFFFF'>IMPORTE EN <span id='monedaSpan'>".$cotizacion['coti'][0]['moneda']."</span></td>".
                                  " <td width='3%'></td>".
			...
			foreach($coti_cargos as $detalle)
			{
                          ...
                          if ( $detalle['iva'] == "1")
                             { $ivaStr = "checked";  }
			  $numca     = $numca + 1;
                          $totCargos = $totCargos + $detalle['importe'];
			  $myRow = $myRow." <tr>".
			  ...
			  "<td width='30%' align='center'>".number_format($totCargos, 2, '.', ',')."</td>".
			...
			$servicios = $servicios.$myRow."</table>".$saltoLinea.
				  ...
				  " <tr><td width='10%'></td><td width='80%' style='font-weight:bold; background-color:#DC2428; color:#FFFFFF' colspan='2'>NOTAS</td>".
				  " <td width='10%' align='center'>".
		 ...
		 else { $servicios = $servicios."<div id='servicio".$id."'></div>"; }
	...
	$this->table->add_row($celda);
	$celda = array('data' => RAZON_SOCIAL.br(2),'class'=>"CotiIzquierdaBold");	
	$this->table->add_row($celda);
	...
	$this->table->add_row($celda);
	$celda = array('data' => '<a href="'.base_url().'"> <img border=0 src="'.base_url().'images/inicio.png"/> '.URL_SITE.'</a>'.br(2),'class'=>"CotiIzquierdaBold");	
	$this->table->add_row($celda);
	...
	$celda2 = array('data'=>'<span id="generaCotiPDF"> </span>','align'=>"left",'width'=>"50%");						  
